<template>
  <div class="contributionsRadar">
    ...
    <div class="emptyChart" v-else>
      <PieChart />
      <p>
        You have no experiments yet, or your experiments don't declare
...
import { contributionsForExperiments, loadCfExp } from "@/stores/series";
...
const cfExpStore = useStore(contributionsForExperiments);
const isLoading = computed(() => {
  return (
    isMounting.value ||
    cfExpStore.value.state === "empty" ||
    cfExpStore.value.state === "loading"
  );
});
...
async function getData(): Promise<void> {
  await loadCfExp();
  if (cfExpStore.value.datasets.length === 0) {
    hasData.value = false;
  } else {
    hasData.value = true;
    chartData.value.labels = [...cfExpStore.value.labels];
    chartData.value.datasets[0].data = [...cfExpStore.value.datasets[0].data];
  }
}
